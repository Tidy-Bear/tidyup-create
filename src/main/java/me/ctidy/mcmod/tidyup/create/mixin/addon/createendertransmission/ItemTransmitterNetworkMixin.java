...
/**
 * ItemTransmitterNetworkMixin
 *
 * @author Tidy-Bear
 * @since 2024/8/20
 */
@Mixin(targets = "com/forsteri/createendertransmission/blocks/MatterTransmitterNetwork$1", remap = false)
@Pseudo
public abstract class ItemTransmitterNetworkMixin {

    @Redirect(method = "onContentsChanged", at = @At(value = "INVOKE", target = "Lcom/forsteri/createendertransmission/blocks/MatterWorldSavedData;setDirty()V", remap = true))
    private void markDataDirty(MatterWorldSavedData data) {
        if (data != null) {
            data.setDirty();
        }
    }
